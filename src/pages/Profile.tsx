...
import { User, Dumbbell, Brain, Heart, Zap, Shield, Search, Menu, Copy, LogOut } from 'lucide-react';
        ...
        <div className={cn(
          ...
          <div className="flex justify-between items-start mb-8">
            <div className="text-right">
              ...
              <h2 className="text-2xl font-black text-foreground">{gameState.playerName}</h2>
              <p className="text-primary text-xs font-mono opacity-70">ID: {profile?.player_id?.slice(0, 12) || '-------'}</p>
            </div>
            <div className={cn(
              "w-20 h-20 rounded-2xl bg-gray-800/50 border-2 flex items-center justify-center shadow-inner",
              rank.border
            )}>
              <span className={cn("text-5xl font-black italic", rank.color)}>{rank.name}</span>
            </div>
          </div>
          ...
          <div className="flex-1 space-y-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[10px] font-bold uppercase tracking-widest">Abilities</span>
              <div className="flex-1 h-[1px] bg-primary/20" />
            </div>
            
            <div className="grid grid-cols-1 gap-3">
              {stats.map((stat) => (
                <div key={stat.key} className="flex items-center gap-4 bg-muted/20 p-3 rounded-xl border border-white/5">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-end">
                      ...
                      <span className="text-sm font-black">{gameState.levels[stat.key]}</span>
                    </div>
          ...
          <div className="mt-8 flex flex-col items-center gap-4 bg-white/5 p-6 rounded-2xl border border-white/5">
            ...
            <Button 
              ...
              className="text-[10px] text-muted-foreground hover:text-primary transition-colors flex gap-2"
            >
              <Copy className="w-3 h-3" /> COPY HUNTER ID
            </Button>
          </div>
