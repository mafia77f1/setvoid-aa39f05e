...
const Boss = () => {
  ...
  // قائمة جميع الرتب من سولو ليفلينج
  const gates = [
    ...
    <div className="min-h-screen bg-[#020203] text-white font-sans selection:bg-purple-500/30 pb-40 overflow-x-hidden">
                ...
                <div className="space-y-4 pt-4">
                  ...
                  <div className="mt-2 py-2 px-3 bg-white/5 border-l-2 border-white/20">
                    <p className="text-[9px] text-slate-500 font-bold italic leading-relaxed uppercase tracking-tighter">
                      {isHidden ? "?" : "Dimensional crack confirmed. Tap the portal above to initiate transition sequence."}
                    </p>
                  </div>
                </div>
              </div>
      ...
      {selectedGate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className={cn("relative w-full max-w-md rounded-2xl border-2 overflow-hidden", getGateBorderColor(selectedGate.color))} style={{ boxShadow: getGateGlow(selectedGate.color) }}>
            <div className={cn("absolute inset-0 opacity-30 bg-gradient-to-b", getGateColor(selectedGate.color))} />
            <div className="absolute inset-0 bg-[#0c0c0e]/95" />
            
            <div className="relative z-10 p-6">
              <button onClick={() => { setSelectedGate(null); setShowWarning(false); }} className="absolute top-4 left-4 p-2 rounded-full bg-white/10 hover:bg-white/20 transition-colors">
                <X className="w-4 h-4 text-white" />
              </button>
              
              <div className="text-center mb-6">
                <div className={cn("w-20 h-20 mx-auto rounded-xl flex items-center justify-center text-4xl font-black mb-3 bg-gradient-to-br text-white", getGateColor(selectedGate.color))}>
                ...
                <div className="mb-4 p-4 rounded-lg bg-red-500/10 border-2 border-red-500/50 animate-pulse">
                  <div className="flex items-start gap-3 text-red-500">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] leading-tight opacity-80 uppercase">
                        Insufficient power level. Analysis failed.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <button
                ...
                className={cn("w-full py-4 rounded-xl font-bold text-lg transition-all duration-300 text-white bg-gradient-to-r shadow-lg", getGateColor(selectedGate.color))}
              >
