      ...
      <main className="relative z-10 max-w-md mx-auto space-y-6">
        ...
        {activeTab === 'equipment' && (
          <div className="space-y-12 animate-in fade-in slide-in-from-right-4 duration-500 relative">
            ...
            {/* XP Item Detail Card (Modal) */}
            {selectedXpItem && (
              <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
                <div className="relative bg-slate-900 border-2 border-blue-500/50 p-6 w-full max-w-xs shadow-[0_0_40px_rgba(59,130,246,0.4)] animate-in zoom-in-95 duration-200">
                  <button onClick={() => setSelectedXpItem(null)} className="absolute -top-3 -right-3 bg-blue-600 p-1 rounded-full border border-white/20">
                    <X className="w-4 h-4 text-white" />
                  </button>
                  
                  <div className="text-center mb-6 border-b border-blue-500/20 pb-4">
                    <h3 className="text-blue-400 font-bold tracking-widest uppercase italic text-sm">{selectedXpItem.name}</h3>
                    <div className="inline-block mt-2 px-3 py-0.5 bg-blue-500/20 border border-blue-400/30 rounded-full">
                      <p className="text-[9px] text-blue-200 font-bold uppercase tracking-tighter">Experience Boost: +{selectedXpItem.xpAmount || 100}</p>
                    </div>
                  </div>
                    ...
                    {[
                      ...
                      <div key={stat.key} className="space-y-1.5">
                        ...
                        <div className="h-1 bg-slate-800 rounded-full overflow-hidden w-full">
                          <div className="h-full bg-blue-400 shadow-[0_0_8px_rgba(96,165,250,0.6)]" style={{ width: '45%' }} />
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <button 
                    ...
                    className="w-full py-3 bg-blue-600/20 border border-blue-500/50 text-blue-100 text-[10px] font-bold tracking-[0.3em] uppercase hover:bg-blue-600/40 transition-all active:scale-95"
                  >
                    Consume Item
                  </button>
                </div>
                  ...
                  <div className="relative bg-black/60 border-2 border-slate-200/90 p-4 shadow-[0_0_20px_rgba(30,58,138,0.3)]">
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="w-20 h-20 border border-slate-500/50 flex items-center justify-center bg-black/40 relative flex-shrink-0 overflow-hidden">
                          {item.name === "Mana Gauge" ? (
                            <img 
                              ...
                              alt="Mana Gauge" 
                              className="w-full h-full scale-125 object-contain filter drop-shadow-[0_0_8px_rgba(59,130,246,0.5)]" 
                            />
                          ) : (
                      ...
                      {item.type === 'title' ? (
                        ...
                        <button
                          onClick={() => {
                            if (item.type === 'experience' || item.name?.toLowerCase().includes('book')) {
