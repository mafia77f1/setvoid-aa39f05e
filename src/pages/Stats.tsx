...
const Stats = () => {
  ...
  // دالة التحليل التي تستهلك جهاز المانا
  const openAnalysis = (item: any) => {
    const manaGauge = gameState.inventory.find(i => (i.id === 'mana_meter' || i.name === 'Mana Gauge') && i.quantity > 0);
    
    if (!manaGauge) {
      toast({ title: "System Error", description: "Requires Mana Gauge to scan items.", variant: "destructive" });
      return;
    }

    useItem(manaGauge.id); // كسر جهاز المانا
    setActiveItem(item);
    setIsScanning(true);
    setIsExiting(false);
          ...
          <div className={cn(
            ...
            <div className={cn("p-6 space-y-6 transition-all duration-1000 delay-700", isVisible && !isExiting ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4")}>
              <div className="flex justify-between items-center border-b border-blue-500/30 pb-2">
                ...
                <h2 className="text-blue-400 text-sm font-bold tracking-[0.2em] uppercase italic">{isUsing ? 'Item Usage' : 'System Analysis'}</h2>
              ...
              {/* 2. XP Book Special Interaction */}
              {isUsing && activeItem.type === 'experience' && (
                <div className="bg-black/40 border border-slate-700/50 p-4 space-y-4">
                   <div className="flex items-center gap-2 border-l-2 border-green-500 pl-2">
                    ...
                    <span className="text-[10px] font-bold text-green-100 uppercase italic">Stat Enhancement</span>
                  </div>
                  
                  <div className="space-y-2">
                    <p className="text-[9px] text-slate-500 uppercase tracking-widest text-center">Set Quantity (Available: {activeItem.quantity})</p>
                    <input 
                      type="number" min="1" max={activeItem.quantity} value={itemQuantity}
                      onChange={(e) => setItemQuantity(Math.max(1, parseInt(e.target.value)))}
                      className="w-full bg-blue-950/40 border border-blue-500/30 text-center text-blue-100 py-1 font-bold"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {stats.map(s => (
                      <button 
                        key={s.category} onClick={() => setSelectedStat(s.category)}
                        className={cn("py-2 text-[9px] border transition-all uppercase font-bold", selectedStat === s.category ? "bg-blue-600 border-white text-white" : "bg-slate-900 border-slate-700 text-slate-500")}
                      >
                        {s.name} {itemQuantity > 1 && <span className="text-green-400 ml-1">+{itemQuantity * 10}</span>}
                      </button>
                    ))}
                  </div>

                  <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden relative">
                    <div className="h-full bg-blue-500 transition-all duration-500" style={{ width: `${getXpProgress(gameState.stats[selectedStat as keyof typeof gameState.stats])}%` }} />
                    <div className="h-full bg-green-400 absolute top-0 opacity-50 animate-pulse" style={{ width: `${Math.min(100, getXpProgress(gameState.stats[selectedStat as keyof typeof gameState.stats]) + (itemQuantity * 5))}%` }} />
                  </div>
                  <p className="text-center text-green-400 text-[9px] font-bold animate-pulse tracking-widest">▲ ENHANCEMENT PREVIEW ACTIVE</p>
                </div>
              )}

              {/* 3. Acquisition Card (Analaysis only) */}
              {!isUsing && (
                <div className="bg-black/40 border border-slate-700/50 p-4 space-y-3">
            ...
            {gameState.inventory.filter(i => i.quantity > 0).map((item, index) => (
                <div key={`${item.id}-${index}`} className="relative bg-black/60 border-2 border-slate-200/90 p-4 shadow-[0_0_20px_rgba(30,58,138,0.3)] transition-all active:scale-[0.98]">
                  ...
                  <button 
                    ...
                    className="absolute -top-3 -left-3 z-20 w-10 h-10 bg-slate-400 border-2 border-black text-black font-black text-xl flex items-center justify-center shadow-lg hover:bg-slate-200 active:scale-90 transition-all"
                  >
                    ❓
                  </button>
