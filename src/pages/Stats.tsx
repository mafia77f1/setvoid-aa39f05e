...
const Stats = () => {
  ...
  const stats = [
    { category: 'strength', level: gameState.levels.strength, xp: gameState.stats.strength, xpProgress: getXpProgress(gameState.stats.strength), name: 'STRENGTH', icon: <Dumbbell className="w-5 h-5" />, color: '#60a5fa' },
    { category: 'mind', level: gameState.levels.mind, xp: gameState.stats.mind, xpProgress: getXpProgress(gameState.stats.mind), name: 'MIND', icon: <Brain className="w-5 h-5" />, color: '#60a5fa' },
    { category: 'spirit', level: gameState.levels.spirit, xp: gameState.stats.spirit, xpProgress: getXpProgress(gameState.stats.spirit), name: 'SPIRIT', icon: <Heart className="w-5 h-5" />, color: '#60a5fa' },
    { category: 'agility', level: gameState.levels.agility || 0, xp: gameState.stats.agility || 0, xpProgress: getXpProgress(gameState.stats.agility || 0), name: 'AGILITY', icon: <Zap className="w-5 h-5" />, color: '#60a5fa' },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#020817] text-white p-3 font-sans selection:bg-blue-500/30 pb-24">
      ...
      <main className="relative z-10 max-w-md mx-auto space-y-6">
                ...
                {stats.map((stat) => (
                  <div key={stat.category} className="bg-black/60 border border-slate-700/50 p-3 hover:border-blue-500/50 transition-colors">
                    <div className="flex justify-between items-end mb-2">
                      ...
                      <span className="text-lg font-black italic text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.4)]">{stat.level}</span>
                    </div>
                    <div className="h-1 bg-slate-800 rounded-full overflow-hidden">
                      <div className="h-full bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.8)] transition-all duration-1000" style={{ width: `${stat.xpProgress}%` }} />
                    </div>
                  </div>
                ))}
              </div>
        ...
        {activeTab === 'equipment' && (
          <div className="space-y-12 animate-in fade-in slide-in-from-right-4 duration-500 relative">
            {/* Modal Card for Experience Items */}
            {selectedXpItem && (
              <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
                <div className="relative bg-slate-900 border-2 border-blue-500/50 p-6 w-full max-w-sm shadow-[0_0_30px_rgba(59,130,246,0.2)]">
                  <button onClick={() => setSelectedXpItem(null)} className="absolute top-2 right-2 text-slate-400 hover:text-white"><X className="w-5 h-5" /></button>
                  <div className="text-center mb-6">
                    <h3 className="text-blue-400 font-bold tracking-widest uppercase italic">{selectedXpItem.name}</h3>
                    <p className="text-[10px] text-slate-400 font-mono mt-1">XP AMOUNT: +{selectedXpItem.xpAmount || 500}</p>
                  </div>
                  
                  <div className="grid grid-cols-2 gap-4 mb-6">
                    {[
                      { label: 'القوة', key: 'strength' },
                      { label: 'العقل', key: 'mind' },
                      { label: 'الروح', key: 'spirit' },
                      { label: 'اللياقة', key: 'agility' }
                    ].map((f) => (
                      <div key={f.key} className="space-y-1">
                        <div className="flex justify-between text-[10px] font-bold">
                          <span className="text-slate-300">{f.label}</span>
                          <span className="text-blue-400">+{selectedXpItem.xpAmount || 500}</span>
                        </div>
                        <div className="h-1 bg-slate-800 w-full overflow-hidden">
                          <div className="h-full bg-blue-500 shadow-[0_0_5px_#3b82f6]" style={{ width: '40%' }} />
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <button 
                    onClick={() => { useItem(selectedXpItem.id); setSelectedXpItem(null); }}
                    className="w-full py-2 bg-blue-600 text-white text-[10px] font-bold tracking-[0.2em] uppercase hover:bg-blue-500 transition-colors"
                  >
                    CONFIRM CONSUMPTION
                  </button>
                </div>
                      ...
                      <div className="flex items-center gap-4">
                        <div className="w-20 h-20 border border-slate-500/50 flex items-center justify-center bg-black/40 relative flex-shrink-0">
                          {item.name === "Mana Gauge" ? (
                             <img src="/ManaDeviceIcon.png" alt="Mana Gauge" className="w-14 h-14 object-contain" />
                          ) : (
                      ...
                      {item.type === 'title' ? (
                        <button
                          ...
                          className={cn(
                            ...
                            item.equipped ? "bg-yellow-500/10 border-yellow-500/40 text-yellow-300" : "bg-blue-500/10 border-blue-500/40 text-blue-300 hover:bg-blue-500/20"
                          )}
                        >
                        ...
                        <button
                          onClick={() => item.type === 'experience' ? setSelectedXpItem(item) : useItem(item.id)}
