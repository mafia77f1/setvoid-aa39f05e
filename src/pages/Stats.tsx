                ...
                {stats.map((stat) => (
                  <div key={stat.category} className="bg-black/60 border border-slate-700/50 p-3 hover:border-blue-500/50 transition-colors">
                    <div className="flex justify-between items-end mb-2 text-white">
                      ...
                      <span className="text-lg font-black italic drop-shadow-[0_0_8px_rgba(255,255,255,0.4)]">{stat.level}</span>
                    </div>
          ...
          <div className="space-y-12 animate-in fade-in duration-500 relative">
            ...
            {selectedXpItem && (
              <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/90 backdrop-blur-md">
                <div className="relative bg-[#0a0f1d] border-2 border-blue-500/50 p-6 w-full max-w-sm shadow-[0_0_50px_rgba(29,78,216,0.5)]">
                  <button onClick={() => setSelectedXpItem(null)} className="absolute top-2 right-2 text-slate-400"><X className="w-5 h-5" /></button>
                  <div className="text-center mb-6">
                    <h3 className="text-blue-400 font-black tracking-widest uppercase italic">{selectedXpItem.name}</h3>
                    <p className="text-[10px] text-blue-200 mt-1 font-bold">XP BOOST: +{selectedXpItem.xpAmount || 100}</p>
                  </div>
                  <div className="grid grid-cols-2 gap-4 mb-8">
                    {[{l:'القوة',k:'s'}, {l:'العقل',k:'m'}, {l:'الروح',k:'sp'}, {l:'اللياقة',k:'a'}].map((s) => (
                      <div key={s.l} className="space-y-2">
                        <p className="text-[10px] font-bold text-slate-300">{s.l}</p>
                        <div className="h-1 bg-slate-800 w-full"><div className="h-full bg-blue-500 shadow-[0_0_10px_#3b82f6]" style={{width:'60%'}} /></div>
                      </div>
                    ))}
                  </div>
                  <button onClick={() => { useItem(selectedXpItem.id); setSelectedXpItem(null); }} className="w-full py-3 bg-blue-600/20 border border-blue-500 text-blue-100 text-[10px] font-black uppercase tracking-widest hover:bg-blue-600/40">CONSUME ITEM</button>
                </div>
              </div>
            )}
                ...
                <div key={`${item.id}-${index}`} className="relative bg-black/60 border-2 border-slate-200/90 p-4">
                  ...
                  <div className="flex flex-col gap-4">
                    <div className="flex items-center gap-4">
                      <div className="w-20 h-20 border border-slate-500/50 flex items-center justify-center bg-black/40 flex-shrink-0 overflow-hidden">
                        {item.name === "Mana Gauge" ? (
                          <img src="/ManaDeviceIcon.png" alt="Mana Gauge" className="w-[140%] h-[140%] object-contain scale-110" />
                        ) : (
                    ...
                    <button
                      onClick={() => {
                        if (item.type === 'experience' || item.name.toLowerCase().includes('كتاب')) {
                      ...
                      className="w-full mt-2 py-2 bg-blue-500/10 border border-blue-500/40 text-blue-300 text-[10px] font-bold uppercase tracking-[0.2em] active:scale-[0.95]"
                    >
