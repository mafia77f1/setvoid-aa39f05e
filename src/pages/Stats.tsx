        ...
        {activeTab === 'stats' ? (
          <div className="space-y-8 animate-in fade-in duration-500">
            <div className="relative bg-black/60 border-2 border-slate-200/90 p-6 shadow-[0_0_20px_rgba(30,58,138,0.3)]">
              ...
              <div className="flex flex-col items-center gap-2">
                ...
                <div className="text-[10px] font-bold tracking-[0.4em] uppercase py-1 px-4 border-y border-white/20" style={{ color: levelConfig.color }}>
                  {levelConfig.tier}
                </div>
              </div>
            </div>
          ...
          <div className="space-y-12 animate-in fade-in duration-500 pt-4">
            {gameState.inventory.length === 0 ? (
              <div className="text-center py-20 border-2 border-dashed border-slate-800 text-slate-500 uppercase text-[10px] font-bold tracking-widest">Inventory Empty</div>
            ) : (
                    ...
                    <div className="flex flex-col gap-4">
                      <div className="flex items-center gap-4">
                        <div className="w-24 h-24 border border-slate-500/50 flex items-center justify-center bg-black/40 relative flex-shrink-0">
                          ...
                          <div className="absolute -bottom-2 -right-2 bg-blue-600 border border-blue-400 px-2 py-0.5 text-[10px] font-bold">x{item.quantity || 1}</div>
                        </div>

                        <div className="flex-1 space-y-2">
                          <div className="flex justify-between items-center border-b border-white/10 pb-1">
                            <p className="text-[9px] text-slate-400 uppercase font-bold">Rank:</p>
                            <p className="text-xs font-bold text-white italic uppercase">{item.difficulty || '?'}</p>
                          </div>
                          <div className="flex justify-between items-center border-b border-white/10 pb-1">
                            <p className="text-[9px] text-slate-400 uppercase font-bold">Cat:</p>
                            <p className="text-xs font-bold text-white italic uppercase">{item.category}</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <button
                        ...
                        className="w-full mt-2 py-2 text-[10px] font-bold tracking-[0.2em] uppercase border bg-blue-500/10 border-blue-500/40 text-blue-300 hover:bg-blue-500/20 active:scale-[0.95]"
                      >
                        Use Item
                      </button>
                    </div>
